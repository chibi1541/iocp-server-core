...
namespace SP
{
	...
	// 테이블의 Column 정보를 전부 가져오는데 필요한 Param을 바인딩한 클래스
...
// GetDBTables를 통해서 쿼리를 날려 DB로부터 DB구조를 뽑아와서 _dbTables에 저장
...
void DBSynchronizer::CompareDBModel()
{
	...
	for (DBModel::TableRef& dbTable : _dbTables)
	{
		...
		else
		{
			...
			{
				LOG_WARN(L"Removing Table : [dbo].[%s]", dbTable->_name.c_str());
	...
	for (auto& mapIt : xmlTableMap)
	{
		...
		LOG_WARN(L"Creating Table : [dbo].[%s]", xmlTable->_name.c_str());
		...
		{
			LOG_WARN(L"Creating Index : [%s] %s %s [%s]", xmlTable->_name.c_str(), xmlIndex->GetKeyText().c_str(), xmlIndex->GetTypeText().c_str(), xmlIndex->GetUniqueName().c_str());
...
void DBSynchronizer::CompareTables(DBModel::TableRef dbTable, DBModel::TableRef xmlTable)
{
	...
	for (DBModel::ColumnRef& dbColumn : dbTable->_columns)
	{
		...
		{
			LOG_WARN(L"Dropping Column : [%s].[%s]", dbTable->_name.c_str(), dbColumn->_name.c_str());
	...
	for (auto& mapIt : xmlColumnMap)
	{
		...
		LOG_WARN(L"Adding Column : [%s].[%s]", dbTable->_name.c_str(), xmlColumn->_name.c_str());
	...
	for (DBModel::IndexRef& dbIndex : dbTable->_indexes)
	{
		...
		{
			LOG_WARN(L"Dropping Index : [%s] [%s] %s %s", dbTable->_name.c_str(), dbIndex->_name.c_str(), dbIndex->GetKeyText().c_str(), dbIndex->GetTypeText().c_str());
	...
	for (auto& mapIt : xmlIndexMap)
	{
		...
		LOG_WARN(L"Creating Index : [%s] %s %s [%s]", dbTable->_name.c_str(), xmlIndex->GetKeyText().c_str(), xmlIndex->GetTypeText().c_str(), xmlIndex->GetUniqueName().c_str());
		if (xmlIndex->_primaryKey || xmlIndex->_uniqueConstraint)
		{
...
void DBSynchronizer::CompareColumns(DBModel::TableRef dbTable, DBModel::ColumnRef dbColumn, DBModel::ColumnRef xmlColumn)
{
	...
	if (flag)
	{
		LOG_WARN(L"Updating Column [%s] : (%s) -> (%s)", dbTable->_name.c_str(), dbColumn->CreateText().c_str(), xmlColumn->CreateText().c_str());
	}
...
void DBSynchronizer::CompareStoredProcedures()
{
	...
	for (DBModel::ProcedureRef& dbProcedure : _dbProcedures)
	{
		...
		if (findProcedure != xmlProceduresMap.end())
		{
			...
			{
				LOG_WARN(L"Updating Procedure : %s", dbProcedure->_name.c_str());
	...
	{
		LOG_WARN(L"Updating Procedure : %s", mapIt.first.c_str());
